...
func (r *queryResolver) BoardByName(ctx context.Context, name string, viewerID *uuid.UUID) ([]*model.Board, error) {
	logger := zap.L().With(zap.String("resolver", "BoardByName"), zap.String("name", name))
	logger.Info("Fetching boards by name")

	boards, err := r.BoardRepo.GetByName(ctx, name, viewerID)
	if err != nil {
...
func (r *queryResolver) BoardsByGroup(ctx context.Context, groupID uuid.UUID, viewerID *uuid.UUID) ([]*model.Board, error) {
	...
	boards, err := r.BoardRepo.GetByGroup(ctx, groupID, viewerID)
	if err != nil {
		logger.Error("Failed to fetch boards", zap.Error(err))
		return nil, err
	}
...
func (r *queryResolver) PinsByUser(ctx context.Context, userID uuid.UUID, viewerID *uuid.UUID) ([]*model.Pin, error) {
	logger := zap.L().With(zap.String("resolver", "PinsByUser"), zap.String("userID", userID.String()))
	logger.Info("Fetching pins by user")

	pins, err := r.PinRepo.GetByUser(ctx, userID, viewerID)
	if err != nil {
...
func (r *queryResolver) PinsByName(ctx context.Context, name string, viewerID *uuid.UUID) ([]*model.Pin, error) {
	logger := zap.L().With(zap.String("resolver", "PinsByName"), zap.String("name", name))
	logger.Info("Fetching pins by name")

	pins, err := r.PinRepo.GetByName(ctx, name, viewerID)
	if err != nil {
		logger.Error("Failed to fetch pins", zap.Error(err))
		return nil, err
	}
...
func (r *queryResolver) PinsByLocation(ctx context.Context, query string, viewerID *uuid.UUID) ([]*model.Pin, error) {
